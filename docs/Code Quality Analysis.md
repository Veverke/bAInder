...
| `lib/chat/annotations.js` | 1.5% | 0% | 0% | 1.8% | ❌ Effectively untested |
| `lib/dialogs/chat-dialogs.js` | 100% | 94.5% | 100% | 100% | ✅ Excellent |
| `lib/dialogs/chat-manager.js` | 100% | 100% | 100% | 100% | ✅ Excellent |
| `lib/dialogs/dialog-manager.js` | 100% | 97.2% | 100% | 100% | ✅ Excellent |
| `lib/dialogs/export-dialog.js` | — | — | — | — | ⚠️ Test file failing (excluded) |
| `lib/dialogs/export-engine.js` | 92.8% | 76.1% | 92.6% | 95.5% | ✅ Good; branch gaps in HTML/ZIP paths |
| `lib/dialogs/import-dialog.js` | — | — | — | — | ⚠️ Test file failing (excluded) |
| `lib/import-parser.js` | 93.8% | 77.6% | 100% | 96.5% | ✅ Good |
| `lib/utils/logger.js` | — | — | — | — | ❌ No tests |
| `lib/io/markdown-serialiser.js` | 97.9% | 97.3% | 100% | 98.8% | ✅ Excellent |
| `lib/utils/search-utils.js` | 57.4% | 44.2% | 50.0% | 57.4% | ⚠️ Low — filter functions uncovered |
| `lib/sticky-notes/sticky-notes-ui.js` | 30.4% | 36.9% | 22.2% | 30.5% | ❌ Low coverage |
| `lib/sticky-notes/sticky-notes.js` | 100% | 91.7% | 100% | 100% | ✅ Excellent |
...
| `lib/theme/useTheme.js` | — | — | — | — | ❌ No tests |
...
| ~~1.2~~ | ~~HTML-escape implemented 4 separate times~~ | ~~`src/lib/utils/search-utils.js`, `src/reader/reader.js`, `src/lib/style-transformer.js`, `src/lib/dialog-manager.js`, `src/lib/export-engine.js` as `_esc()`~~ | ✅ **RESOLVED** — `escapeHtml` in `src/lib/utils/search-utils.js` is now the canonical implementation (null-safe, 5-char set including `&#39;`). All four duplicates removed: `reader.js` imports and re-exports it; `style-transformer.js` imports it (replacing the nested `_escapeHtml`); `dialog-manager.escapeHtml()` delegates to it (replacing the DOM-based approach); `export/format-helpers.esc()` wraps it (replacing the inline regex chain). One definition, five consumers. |
| ~~1.3~~ | ~~ID generation pattern repeated 4 times~~ | ~~`src/lib/tree.js` (Topic), `src/lib/tree.js` (ChatEntry), `src/background/chat-save-handler.js`, `src/reader/reader.js`~~ | ✅ **RESOLVED** — `generateId(prefix?)` added to `src/lib/utils/search-utils.js` as the canonical implementation. With a prefix (e.g. `'topic'`, `'chat'`, `'ann'`) produces `{prefix}_{ts}_{random}`; without a prefix produces `{ts}-{random}` (preserving the format expected by the `generateChatId` test). `Topic` and `ChatEntry` in `models.js` now call `generateId('topic'/'chat')` in their constructors; `generateChatId()` in `chat-save-handler.js` delegates to `generateId()`; annotation IDs in `reader.js` use `generateId('ann')`. One definition, four consumers. |
...
| ~~7.3~~ | ~~Predictable IDs used to key all data~~ | ~~`src/lib/utils/search-utils.js`~~ | ✅ **RESOLVED** — `generateId()` in `src/lib/utils/search-utils.js` now uses `crypto.getRandomValues(new Uint8Array(6))` to produce a 12-char hex string (48 bits of entropy) as the random segment, replacing `Math.random().toString(36)`. The structural format `{prefix}_{ts}_{hex}` / `{ts}-{hex}` is unchanged so all callers and the `generateChatId` timestamp-parse test continue to pass. `crypto.getRandomValues` is always available in Chrome extension contexts (background, content, sidepanel) and in jsdom/Vitest. All 1,217 tests pass (no regressions). |
...
>
> **Refactor session (2026-03-04, continued):** Resolved issue 7.3 — replaced `Math.random().toString(36)` in `generateId()` (`src/lib/utils/search-utils.js`) with `crypto.getRandomValues(new Uint8Array(6))` producing a 12-char hex random segment (48 bits of entropy). IDs are now cryptographically unpredictable even when the timestamp is known. Format is structurally unchanged; all callers and the `generateChatId` timestamp test pass. All 1,217 tests pass (no regressions).
>
